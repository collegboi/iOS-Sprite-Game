...
class GameScene: SKScene, AnalogControlPositionChange {
    ...
    func analogControlPositionChanged(analogControl: AnalogControl, position: CGPoint)  {
        ...
        playerPos = CGPoint(x: player.sprite.position.x, y: player.sprite.position.y)
        
       // centerViewOn(playerPos)
    ...
    func createWorld() {
        ...
        let bounds = SKNode()
        bounds.physicsBody = SKPhysicsBody(edgeLoopFromRect: CGRect(x: 0, y: 0,
            width: backGroundLayer.layerSize.width, height: backGroundLayer.layerSize.height))
